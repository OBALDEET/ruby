def read_file (age = gets.to_i)
  file = File.open("Lab3/input.txt")
  file_data = file.readlines.map{|line| line.chomp}
  file.close

  res_file = File.open("Lab3/result.txt", "w")
  ...
  result_file = File.open("Lab3/result.txt")
...
def open_file
  if File.exist?("Lab3/balance.txt")
    file = File.open("Lab3/balance.txt")
  else
    file = File.open("Lab3/balance.txt", "w+")
    file.puts("100")
    file.close
  end
end
...
def deposit
  ...
  file_write = File.open("Lab3/balance.txt", "w")
